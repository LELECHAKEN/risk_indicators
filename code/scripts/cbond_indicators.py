...
def cal_insert_cbond_turnover_avg(cbond_windcode_list: list, beg_date: str, end_date: str, n_days=[2, 5, 10, 20]):
    ...
    turnover_mean = pd.merge(turnover_mean, list_days.stack().to_frame('list_days').reset_index(),
                             left_on=['trade_dt', 'cbond_windcode'], right_on=['trade_dt', 'cbond_windcode'],
                             how='left')
...
class CBondIndicators(RiskIndicators):
    def __init__(self, t, save_path, ptf_codes=None):
        self.basedate = t
        self.basedatetime = datetime.datetime.strptime(self.basedate, '%Y-%m-%d')
    ...
    def get_basic_info(self):
        ...
        # 未上市转债
        new_list = basic_info[
            (basic_info['list_date'].isnull()) | (basic_info['list_date'] > pd.to_datetime(self.basedate))]
        if new_list.shape[0] > 0:
            print(f"持仓中存在未上市转债{new_list['cbond_name'].to_list()} \n {new_list['cbond_windcode'].to_list()}，预警中将剔除...")
            self.CBbond_holdings = CBbond_holdings.loc[
                ~CBbond_holdings['code'].isin(new_list['cbond_windcode'].to_list())]
        pass
    ...
    def get_remain_amount(self):
        ...
        remain_amount_holdings = remain_amount_mat.reindex(
        ...
        remain_amount_holdings = pd.merge(remain_amount_holdings, issue_size, left_index=True, right_on='cbond_code',
                                          how='left')
        remain_amount_holdings['remain_amount'] = remain_amount_holdings['remain'].fillna(
            remain_amount_holdings['actualissuesize'])
    ...
    def get_turnover(self):
        ...
        if turnover_temp.shape[0] > 0:
            ...
            turnover_temp = pd.merge(turnover_temp, self.basic_info[['cbond_code', 'list_date']], on='cbond_code',
                                     how='left')
            turnover_temp['list_date_idx'] = pd.Index(AShareCalendar['trade_datetime']).get_indexer(
                turnover_temp['list_date'])
            ...
            turnover_mean = pd.concat(
                [turnover_mean,
                 turnover_temp.loc[turnover_temp['list_duration'] >= 10, ['cbond_code', 'turnover_mean']]])
    ...
    def cal_monetize_days(self):
        ...
        df = pd.merge(CBbond_holdings, remain_amount[['cbond_code', 'end_date', 'remain_amount']],
                      left_on=['d_date', 'cbond_code'], right_on=['end_date', 'cbond_code'], how='left').drop(
            columns=['end_date'])
        # 换手率（%)
    ...
    def get_deriv_indicators(self):
        ...
        cb_deriv_indicators['cbond_windcode'] = cb_deriv_indicators.apply(
            lambda x: x['cbond_code'] + x['cbond_windcode'], axis=1)
        ...
        cb_deriv_indicators_filt = pd.concat([cb_deriv_indicators_filt,
                                              cb_deriv_indicators[
                                                  ~cb_deriv_indicators['cbond_code'].isin(duplicated_code)]])
    ...
    def monitor_special_clause_risk(self):
        ...
        call_condition['if_call_by_amount'] = ((call_condition['remain_amount'] / 10000)
                                               < call_condition[
                                                   'callunconvertamount'])  # & (call_condition['if_in_call_period'])
    ...
    def monitor_repay_risk(self):
        ...
        res = df.loc[:,
              ['d_date', 'c_fundname', 'cbond_name', 'cbond_windcode', '市场近10个交易日成交量', '变现天数', 'f_asset',
               'f_assetratio',
        ...
        res.loc[res['变现天数'].notnull(), '风险详情'] = res.loc[res['变现天数'].notnull()].apply(
            lambda
                x: f"近10日日均成交量: {round(x['市场近10个交易日成交量'] / 10000 * 100)}万，以{int(self.participate * 100)}%参与度计算，可参与日均成交量为{round(x['市场近10个交易日成交量'] / 10000 * 100 * self.participate)}万，变现需{round(x['变现天数'])}天\n专户剩余到期日：{x['距离到期时间']}天",
            axis=1)
    ...
    def eval_cond_redeem(self):
        ...
        # 赎回公告
        db_jy = DbFactor(config['data_base']['JY']['url'])
        cbond_special_notice = db_jy.get_factor('cbond_redeem_special_notice',
                                                code_list=self.basic_info['cbond_code'].to_list(),
                                                field='s.SECUCODE')
        cbond_special_notice = cbond_special_notice[cbond_special_notice['ann_date'] <=
                                                    TradeDays.offset_date(int(self.basedate.replace('-', '')), 1, 'D',
                                                                          res_type='datetime')[0]]
        cbond_call_notice = db_jy.get_factor('cbond_callput_info', code_list=self.basic_info['cbond_code'].to_list(),
                                             field='s.SECUCODE')
        cbond_call_notice['赎回登记日'] = cbond_call_notice['赎回登记日'].astype('datetime64')
        cbond_call_notice = cbond_call_notice[cbond_call_notice['ann_date'] <=
                                              TradeDays.offset_date(int(self.basedate.replace('-', '')), 1, 'D',
                                                                    res_type='datetime')[0]]
        # 公告数据修正：不行权区间起始日应当在触发条件满足日之后，否则满足日当天直接进入保护期if_stopexe=True，当日不会触发预警
        cbond_call_notice['触发不行权区间起始日'] = np.where(
            cbond_call_notice['触发不行权区间起始日_原始值'] == cbond_call_notice['触发条件满足区间截止日'],
            cbond_call_notice['触发不行权区间起始日_原始值'] + pd.tseries.offsets.DateOffset(),
            cbond_call_notice['触发不行权区间起始日_原始值'])

        # 2025/6/12: 公告数据修正，本次不赎回，但不行权区间起始日无数据的情况：G三峡EB 2025/6/5
        cond_check = (cbond_call_notice['是否行权'] == 2) & (
                    cbond_call_notice['触发不行权区间起始日'].isnull() & ~cbond_call_notice['触发不行权区间截止日'].isnull())
        cbond_call_notice.loc[cond_check, '触发不行权区间起始日'] = cbond_call_notice.loc[cond_check, '触发条件满足区间截止日'] + pd.tseries.offsets.DateOffset()
        ...
        cond_redeem = pd.merge(cond_redeem, self.basic_info[['cbond_innercode', 'stk_code']], on='cbond_innercode',
                               how='left')
        ...
        cbond_convert_price_mat = pd.pivot_table(cond_redeem, values='convert_price', index='tradingday',
                                                 columns='cbond_code')
        # index 对齐
        ...
        cbond_convert_price_mat = cbond_convert_price_mat.reindex(columns=cbond_code_list,
                                                                  index=pd.to_datetime(
                                                                      duration_days['trade_dt'].astype('str')))
        # 2.逐个判断转换条件
        call_condition['if_in_stopexe'] = np.nan  # 是否在不行权区间
        call_condition['reach_con_call_days'] = np.nan
        call_condition['if_call_by_price'] = np.nan  # 是否满足阈值天数
        ...
        for i in range(0, len(cbond_code_list)):
            ...
            # 历史赎回公告
            cbond_call_notice_i = cbond_call_notice[cbond_call_notice['cbond_code'] == cbond_i]
            ...
            cond_1_i = (cbond_stkprice_mat >= (cbond_convert_price_mat * call_level_i))[cbond_i]
            # 在赎回期内
            if_in_callperiod = cond_1_i.to_frame().apply(lambda x:
                                                         (x.index >= call_i['callstartdate'].values[0]) &
                                                         (x.index <= call_i['callenddate'].values[0]))
            # 判断是否在行权保护期内，在为1，不在为0
            cond_2_i = cond_1_i.to_frame('if_stopexe')
            cond_2_i['if_stopexe'] = 0
            for j in range(0, cbond_call_notice_i.shape[0]):
                stopexe_beg_j = cbond_call_notice_i.iloc[j]['触发不行权区间起始日']
                stopexe_end_j = cbond_call_notice_i.iloc[j]['触发不行权区间截止日']
                cont_date_j = cbond_call_notice_i.iloc[j]['触发条件重新计算日']
                if (not all(pd.isnull([stopexe_beg_j, stopexe_end_j, cont_date_j]))) & \
                        (cbond_call_notice_i.iloc[j]['是否行权'] == 2):
                    if not all(pd.isnull([stopexe_beg_j, stopexe_end_j])):
                        cond_2_i = cond_2_i + cond_1_i.to_frame().apply(lambda x:
                                                                        (x.index >= stopexe_beg_j) & (
                                                                                    x.index <= stopexe_end_j)).values
                    else:
                        logger.warning(f"{cbond_i}有不行权保护期但数据库中无具体日期的情况，将进行二次调整")  # 川恒转债两个日期都为空
                        stopexe_beg_j = cbond_call_notice_i.iloc[j]['触发条件满足区间截止日'] + pd.tseries.offsets.DateOffset()
                        stopexe_end_j = cbond_call_notice_i.iloc[j]['触发条件重新计算日'] - pd.tseries.offsets.DateOffset()
                        cond_2_i = cond_2_i + cond_1_i.to_frame().apply(lambda x:
                                                                        (x.index >= stopexe_beg_j) & (
                                                                                    x.index <= stopexe_end_j)).values * 1
            # 最近一个不行权日
            if cond_2_i['if_stopexe'].sum() > 0:
                last_stop_date = cond_2_i.loc[cond_2_i['if_stopexe'] > 0].index[-1]
                last_count_date = TradeDays.offset_date(
                    int(datetime.datetime.strftime(last_stop_date, '%Y%m%d')), 1, 'D', res_type='datetime')[0]
            else:
                last_count_date = cond_2_i.index[0]

            # 在赎回期且满足赎回条件, 从最近一个重新计算日开始数天数
            if last_count_date > self.basedatetime:
                # 当前还在保护期
                # 过去30天中有n日满足
                reach_days_i = 0
                # 是否满足阈值天数
                ...
                reach_days_i = (cond_1_i & if_in_callperiod.iloc[:, 0].values).loc[last_count_date:].rolling(
                    window=int(call_cond_day_i), min_periods=1).sum().iloc[-1]
                if_call_i = reach_days_i >= call_reachday_i
                if_stopexe_i = cond_2_i['if_stopexe'].iloc[-1] > 0
            call_condition.loc[call_condition['cbond_code'] == cbond_i, 'if_in_stopexe'] = if_stopexe_i  # 是否在不行权区间
        ...
        call_condition = pd.merge(call_condition, self.basic_info[['cbond_code', 'list_enddate']], on='cbond_code',
                                  how='left')
        # 在转换期内、退市之前，满足条件赎回天数 + 不在保护期
        call_condition['if_call_by_price_adj'] = (call_condition['if_in_call_period'] &
                                                  (~(call_condition['end_date'] > call_condition['list_enddate']))) & \
                                                 call_condition['if_call_by_price'] & (~call_condition['if_in_stopexe'])
        call_condition_all = self._define_redeem_status(call_condition, cbond_call_notice, cbond_special_notice)
        self.call_condition_all = call_condition_all
        self.cond_redeem = cond_redeem
        pass

    def _define_redeem_status(self, call_condition, cbond_call_notice, cbond_special_notice):
        """
        判断几种赎回状态：
        1）已触发，但公告本次不赎回：'trigger_notcall', '已触发，但本次不赎回'
        2) 已触发，且公告赎回：即if_call_by_price_adj=True的情况, 进一步提取公告; 'trigger_call', '已触发(赎回登记日:yyyy/mm/dd)'
        3) 已触发，但尚未公告: 即if_call_by_price_adj=True的情况; 'trigger', '触发'
        4) 可能满足触发条件(早期预警）,'may_trigger', '可能触发'
        """
        status_mapping = {'trigger': '触发', 'trigger_notcall': '已触发，但本次不赎回',
                          'trigger_call': '已触发(赎回登记日:yyyy/mm/dd)', 'may_trigger': '可能触发'}

        call_condition = call_condition.copy()
        cbond_special_notice = cbond_special_notice.copy()
        list_trigger = call_condition.loc[call_condition['if_call_by_price_adj'], 'cbond_code'].to_list()

        # -------- 1. 触发，但公告本次不赎回（触发条件满足if_call_by_price_adj = True的）:
        # base_dateoffset = TradeDays.offset_date(int(self.basedate.replace('-', '')), -REDEEM_DAYS_BEF, 'D',
        #                                         res_type='datetime')[0]
        # cbond_call_notice['距离上次触发满足天数'] = cbond_call_notice.apply(lambda x:
        #                                                           (base_dateoffset - x['触发条件满足区间截止日']).days, axis=1)
        # call_notice_trigger_notrecall = cbond_call_notice[(cbond_call_notice['距离上次触发满足天数'] <= 0) & \
        #                                            (cbond_call_notice['是否行权'] == 2)]
        call_notice_trigger_notrecall = cbond_call_notice[cbond_call_notice['cbond_code'].isin(list_trigger)
                                                          & (cbond_call_notice['是否行权'] == 2)]
        # 筛选出在不行权区间的
        call_notice_trigger_notrecall = call_notice_trigger_notrecall[
            (call_notice_trigger_notrecall['触发不行权区间截止日'].isnull() |
             (call_notice_trigger_notrecall['触发不行权区间截止日'] >= self.basedatetime))]
        # 删除历史触发数据
        call_notice_trigger_notrecall = call_notice_trigger_notrecall.sort_values(
            by=['cbond_code', '触发条件满足区间截止日'], ascending=False).drop_duplicates(subset=['cbond_code'], keep='first')

        trigger_notcall_info = call_notice_trigger_notrecall.reindex(
            columns=['cbond_code', 'ann_date', '触发条件满足区间截止日', '触发条件重新计算日']).rename(
            columns={'ann_date': 'ann_date_notcall'})
        trigger_notcall_info['call_status'] = 'trigger_notcall'

        # -------- 2 已触发 (川投转债：满足条件日2023/12/29，公告日12/30, 在特别提示表中), 要赎回，提取赎回登记日
        # 提取公告
        cbond_special_notice['ann_date_max'] = cbond_special_notice.groupby('cbond_code')[['ann_date']].transform('max')
        trigger_special_notice = cbond_special_notice.copy()
        # 筛选出最新提示性公告
        trigger_special_notice = trigger_special_notice[trigger_special_notice.apply(lambda x:
                                                                                     (x['ann_date'] == x[
                                                                                         'ann_date_max']) & (
                                                                                                 '已满足连续' in x[
                                                                                             'content']), axis=1)]
        trigger_special_notice = trigger_special_notice.reindex(columns=['cbond_code', 'ann_date', 'content'])
        # 筛选出最新正式公告
        trigger_recall_notice = cbond_call_notice[cbond_call_notice.apply(
            lambda x: (x['赎回登记日'] - self.basedatetime).days >= 0, axis=1)]
        trigger_recall_notice = trigger_recall_notice.reindex(columns=['cbond_code', 'ann_date', '赎回登记日'])
        # 合并上if_call_by_price_adj=True的
        trigger_info = pd.merge(trigger_special_notice, trigger_recall_notice, on='cbond_code', how='outer',
                                suffixes=['_special_note', '_call'])
        trigger_info['call_status'] = trigger_info['赎回登记日'].apply(lambda x: 'trigger' if pd.isnull(x) else
        'trigger_call')

        # -------- 4.可能满足触发条件(早期预警）
        # 提前REDEEM_DAYS_BEF天开始提示, 满足if_call_by_price_adj的剔除
        call_condition['redeem_days_bef_thresh'] = REDEEM_DAYS_BEF
        call_condition['if_call_by_price_adj_bef'] = (call_condition['reach_con_call_days'] >=
                                                      (call_condition['callreachday'] - REDEEM_DAYS_BEF)) & (
                                                                 call_condition['if_in_call_period'] &
                                                                 (~(call_condition['end_date'] > call_condition[
                                                                     'list_enddate']))) & (
                                                         ~call_condition['if_in_stopexe']) & \
                                                     (~call_condition['if_call_by_price_adj'])

        # 结果合并
        # 1~2：已触发的两种情况：公告不赎回及公告赎回
        status = pd.concat([trigger_notcall_info, trigger_info])
        # 删除历史不触发但本次触发的数据
        status['ann_date_all'] = status['ann_date_notcall'].fillna(status['ann_date_call'])
        status = status.sort_values(
            by=['cbond_code', 'ann_date_all'], ascending=False).drop_duplicates(subset=['cbond_code'], keep='first')
        status = status.reindex(columns=['cbond_code', 'ann_date_notcall', '触发条件满足区间截止日', '触发条件重新计算日',
                                         'ann_date_special_note', 'content', 'ann_date_call', '赎回登记日',
                                         'call_status', 'call_status_des'])
        status = status.rename(columns={'触发条件满足区间截止日': 'con_satisf_enddt', '触发条件重新计算日': 'next_recount_dt',
                                        '赎回登记日': 'call_reg_dt', 'content': 'special_note_content'})
        call_condition = pd.merge(call_condition, status, on='cbond_code', how='left')
        call_condition.loc[call_condition['if_call_by_price_adj_bef'] & (~call_condition['if_call_by_price_adj']) &
                           (call_condition['call_status'].isnull()),
                           'call_status'] = 'may_trigger'

        # 3. 剩下满足if_call_by_price_adj=True 已触发但没有公告是否要赎的
        call_condition['call_status'] = np.where(
            call_condition['if_call_by_price_adj'] & pd.isnull(call_condition['call_status']), 'trigger',
            call_condition['call_status'])
        call_condition['call_status_des'] = call_condition['call_status'].replace(status_mapping)
        call_condition['call_status_des'] = call_condition.apply(
            lambda
                x: f"已触发(赎回登记日{datetime.datetime.strftime(x['call_reg_dt'], '%Y/%m/%d')})" if x.call_status == 'trigger_call'
            else x['call_status_des'], axis=1)
    ...
    def _cal_risk_pct(self, bond_risk_data: pd.DataFrame, cond_colname: str):
        ...
        risk_pct = df.groupby(['c_fundname', 'd_date'])[['risk_ratio', 'risk_mv']].sum().reset_index().sort_values(
            by='c_fundname')
    ...
    def _monitor_redeem_impale_risk(self):
        ...
        call_cond_columns = ['cbond_code', 'list_enddate', 'calllevel', 'callconditionday', 'callreachday',
                             'callunconvertamount',
                             'if_call_by_amount', 'if_in_stopexe', 'if_in_call_period', 'reach_con_call_days',
                             'if_call_by_price_adj', 'redeem_days_bef_thresh', 'if_call_by_price_adj_bef',
                             'ann_date_notcall', 'con_satisf_enddt', 'next_recount_dt', 'ann_date_special_note',
                             'special_note_content', 'ann_date_call', 'call_reg_dt', 'call_status', 'call_status_des']
        ...
        # 旧版风险：达到强赎条件且溢价过高, 对应到新版trigger_notcall 、trigger或trigger_call且溢价过高
        redeem_risk['if_redeem_impale_premium'] = redeem_risk['if_call_by_price_adj'] & (
                redeem_risk['convertpremiumrate'] > IMPALE_RISK_THRES)
        redeem_risk.rename(columns={'end_date': 'd_date'}, inplace=True)
        res_redeem_impale_risk, res_redeem_impale_risk_detail = self._cal_risk_pct(
            redeem_risk[['d_date', 'cbond_code', 'if_redeem_impale_premium']],
            'if_redeem_impale_premium')
        res_redeem_impale_risk['risk_type'] = '强赎条款刺破溢价风险'
        res_redeem_impale_risk_detail['risk_type'] = '强赎条款刺破溢价风险'

        # 20240227：
        redeem_risk['if_impale_prem_early'] = redeem_risk['call_status'].isin(
            ['trigger', 'trigger_call', 'trigger_notcall', 'may_trigger']) & (
                                                      redeem_risk['convertpremiumrate'] > IMPALE_RISK_THRES)
        _, res_redeem_impale_risk_detail_early = self._cal_risk_pct(
            redeem_risk[['d_date', 'cbond_code', 'if_impale_prem_early']],
            'if_impale_prem_early')
        res_redeem_impale_risk_detail_early['risk_type'] = '强赎条款刺破溢价风险_含早期'
        self.redeem_risk = redeem_risk
        return res_redeem_impale_risk, res_redeem_impale_risk_detail, res_redeem_impale_risk_detail_early
    ...
    def _monitor_premium_shattered_risk(self):
        ...
        res_premium_shattered_risk, res_premium_shattered_risk_detail = self._cal_risk_pct(
            redeem_risk[['d_date', 'cbond_code', 'if_premium_shattered']],
    ...
    def _monitor_liquidity_risk(self):
        ...
        # # 20220414: 修复bug, liquidity_risk中已经包含组合明细了
        ...
        res_liquidity_risk_detail_columns = ['d_date', 'c_fundname', 'cbond_name', 'f_asset', 'f_assetratio',
                                             'cbond_windcode', 'if_illiquidity', 'risk_ratio', 'risk_mv']
    ...
    def _monitor_speculate_risk(self):
        ...
        20240227：补充条件：过去2/5日换手率超50%，且过去2/5日转债与正股偏离超过5%
        :return:
        """
        speculate_risk = self.monetize_days.copy()
        speculate_risk['if_speculate'] = speculate_risk['turnover_mean'] > 50
        res_speculate_risk, res_speculate_risk_detail = self._cal_risk_pct(
            speculate_risk[['d_date', 'cbond_code', 'if_speculate']],
            'if_speculate')
    ...
    def _get_cbond_quotation(self, beg_date, end_date, code_list):
        """
        获取今日转债行情信息
        :param tdate: 提取日期
        :return:
        """
        db_jy = DbFactor(config['data_base']['JY']['url'])
        cb_quotation = db_jy.get_factor('cbond_market_quotation', code_list=code_list,
                                        field='CI.InnerCode', beg_date=str(beg_date), end_date=str(end_date))
        cb_quotation.drop_duplicates(inplace=True)
        return cb_quotation

    def _cal_bias(self, beg_date: datetime, end_date: datetime, cb_quotation, window_i):
        """
        计算过去window_i日，转债和正股涨跌幅偏离
        """
        quotation = cb_quotation.copy()
        quotation = quotation[quotation['tradingday'].between(beg_date, end_date)]
        # 计算过去window天累计涨跌幅
        cb_quotation_mat = quotation.dropna(subset=['tradingday']).pivot(index='tradingday', columns='cbond_windcode',
                                                                         values='cbond_close')
        cb_quotation_pct_mat = cb_quotation_mat.pct_change()
        cb_rolling_cumret = ((1 + cb_quotation_pct_mat).prod() - 1).to_frame('cb_cumret')

        # 正股累计涨跌幅
        stk_quotation_mat = quotation.dropna(subset=['tradingday']).pivot(index='tradingday',
                                                                          columns='cbond_windcode',
                                                                          values='stk_price')
        stk_quotatio_pct_mat = stk_quotation_mat.pct_change()
        stk_rolling_cumret = ((1 + stk_quotatio_pct_mat).prod() - 1).to_frame('stk_cumret')

        # 计算偏离度
        bias = pd.merge(cb_rolling_cumret,
                        stk_rolling_cumret, left_index=True, right_index=True, how='outer')
        bias['window'] = window_i
        bias['bias'] = bias['cb_cumret'] - bias['stk_cumret']
        return bias.reset_index()

    def _monitor_speculate_risk_new(self, window=[2, 5], thres=[0.05, 0.05]):
        """
        5. 20240227：过去2/5日换手率超50%，且过去2/5日转债与正股偏离超过5%（仅上涨预警）
        :return:
        """
        db_risk = DbFactor(config['data_base']['QUANT']['url'])
        turnover = db_risk.get_factor('rc_cb_turnover_mean', code_list=self.CBbond_holdings['code'].to_list(),
                                      field='cbond_windcode', beg_date=f'\"{self.basedate}\"',
                                      end_date=f'\"{self.basedate}\"')
        turnover = turnover[turnover['method'] == 'SMA'].rename(columns={'n': 'window'})
        cb_quotation = pd.merge(self.basic_info[['cbond_innercode', 'cbond_windcode']], self.cond_redeem,
                                on='cbond_innercode', how='right')
        bias = pd.DataFrame()
        speculate_risk = pd.DataFrame(columns=['cbond_windcode', 'trade_dt'])
        for i in range(0, len(window)):
            # 计算转债与正股近2/5日涨跌幅
            window_i = window[i]
            thresh_i = thres[i]
            # turnover_i = turnover[turnover['n'] == window_i]
            beg_date_i = \
            TradeDays.offset_date(int(self.basedate.replace('-', '')), -window_i, 'D', res_type='datetime')[0]
            bias_i = self._cal_bias(beg_date_i, self.basedatetime, cb_quotation, window_i=window_i)
            bias_i['bias_thres'] = thresh_i
            bias_i['trade_dt'] = self.basedate
            bias_i = pd.merge(bias_i, turnover[['trade_dt', 'cbond_windcode', 'window', 'turnover_mean']],
                              on=['trade_dt', 'cbond_windcode', 'window'], how='left')
            bias_i[f'if_trigger'] = (bias_i['bias'] > bias_i['bias_thres']) & \
                                    (bias_i['turnover_mean'] > SPECULATE_TURNOVER_THRES) & (bias_i['stk_cumret'] > 0)
            speculate_risk_i = bias_i.reindex(columns=['trade_dt', 'cbond_windcode', 'bias',
                                                       'turnover_mean', 'cb_cumret', 'stk_cumret', 'if_trigger'])
            speculate_risk_i = speculate_risk_i.rename(columns={'bias': f'bias_{window_i}',
                                                                'bias_thres': f'bias_thres_{window_i}',
                                                                'turnover_mean': f'turnover_{window_i}',
                                                                'cb_cumret': f'cb_cumret_{window_i}',
                                                                'stk_cumret': f'stk_cumret_{window_i}',
                                                                'if_trigger': f'if_trigger_{window_i}'})
            bias = pd.concat([bias, bias_i])
            speculate_risk = pd.merge(speculate_risk, speculate_risk_i, on=['trade_dt', 'cbond_windcode'],
                                      how='outer')
        speculate_risk['if_speculate'] = speculate_risk['if_trigger_2'] | speculate_risk['if_trigger_5']
        speculate_risk = speculate_risk.rename(columns={'trade_dt': 'd_date'})
        speculate_risk = pd.merge(self.basic_info[['cbond_windcode', 'cbond_code']], speculate_risk,
                                  on='cbond_windcode', how='right')
        speculate_risk['d_date'] = pd.to_datetime(speculate_risk['d_date'])
        res_speculate_risk, res_speculate_risk_detail = self._cal_risk_pct(
            speculate_risk[['d_date', 'cbond_code', 'if_speculate']],
            'if_speculate')
        res_speculate_risk['risk_type'] = '投机炒作风险_含价格偏离'
        res_speculate_risk_detail['risk_type'] = '投机炒作风险_含价格偏离'
        self.speculate_bias = speculate_risk
        return res_speculate_risk, res_speculate_risk_detail, speculate_risk
    ...
    def _monitor_premium_agg(self):
        ...
        res_premium_risk_agg, res_premium_risk_agg_detail = self._cal_risk_pct(
            redeem_risk[['d_date', 'cbond_code', 'if_premium_agg']],
            'if_premium_agg')
    ...
    def monitor_invest_risk(self):
        ...
        # 2. 强赎条款刺破溢价： 满足条件赎回条件，且转股溢价率高于15% -> 5%【占组合净值比例大于10%】
        res_redeem_impale_risk, res_redeem_impale_risk_detail, res_redeem_impale_risk_detail_early = self._monitor_redeem_impale_risk()
        risk_ratio_all = risk_ratio_all.append(res_redeem_impale_risk)
        risk_ratio_detail_all = risk_ratio_detail_all.append(
            res_redeem_impale_risk_detail.drop(columns=['if_redeem_impale_premium']))
        risk_ratio_detail_all = risk_ratio_detail_all.append(
            res_redeem_impale_risk_detail_early.drop(columns=['if_impale_prem_early']))
        ...
        risk_ratio_detail_all = risk_ratio_detail_all.append(
            res_premium_risk_agg_detail.drop(columns=['if_premium_agg']))
        ...
        # 5. 投机炒作风险：近10个交易日平均换手率>50%【占组合净值比例大于10%】
        # 20240227： 过去2/5个交易日换手率超50%, 且过去2/5日转债与正股偏离过大
        res_speculate_risk, res_speculate_risk_detail = self._monitor_speculate_risk()
        res_speculate_risk_new, res_speculate_risk_detail_new, speculate_risk = self._monitor_speculate_risk_new()
        risk_ratio_all = risk_ratio_all.append(res_speculate_risk)
        risk_ratio_detail_all = risk_ratio_detail_all.append(
            res_speculate_risk_detail.drop(columns=['if_speculate']))
        risk_ratio_all = risk_ratio_all.append(res_speculate_risk_new)
        risk_ratio_detail_all = risk_ratio_detail_all.append(
            res_speculate_risk_detail_new.drop(columns=['if_speculate']))
        ...
        risk_ratio_all.loc[risk_ratio_all['risk_type'] == '投机炒作风险_含价格偏离', '风险详情'] = '高换手率且高波动转债持仓总量过高'
    ...
    def monitor_sec_redeem_risk(self):
        ...
        res_sec_redeem_risk = pd.merge(res_sec_redeem_risk, repay_risk_data, on=['d_date', 'c_fundname', 'cbond_name'],
                                       how='left')
    ...
    def _get_rc_alert_cb_port(self):
        ...
        if df_risk.shape[0] > 0:
            ...
            port_risk_i = df_risk.reindex(columns=['d_date', 'risk_type', 'c_fundname', 'cbond_name', 'cbond_windcode',
                                                   'risk_mv', 'risk_ratio', '风险详情']).rename(
                columns={'risk_mv': 'f_asset', 'risk_ratio': 'f_assetratio'})
            port_risk_i = port_risk_i.rename(columns={'风险详情': 'risk_info'})
        ...
        # 202402： 含价格维度的炒作风险
        risk_ratio_all = self.risk_ratio_all.copy()
        cond = (risk_ratio_all['risk_type'] == '投机炒作风险_含价格偏离') & (risk_ratio_all['是否触发'])
        df_risk = risk_ratio_all[cond]
        if df_risk.shape[0] > 0:
            df_risk['risk_type'] = '炒作风险_含价格偏离'
            port_risk_i = df_risk.reindex(columns=['d_date', 'risk_type', 'c_fundname', 'cbond_name', 'cbond_windcode',
                                                   'risk_mv', 'risk_ratio', '风险详情']).rename(
                columns={'risk_mv': 'f_asset', 'risk_ratio': 'f_assetratio'})
            port_risk_i = port_risk_i.rename(columns={'风险详情': 'risk_info'})
    ...
    def get_ratings(self, cbond_innercode_list: list):
        """
        提取外部评级数据
        :param quotation:
        :return:
        ...
        # 提取外部评级
        db_jy = DbFactor(config['data_base']['JY']['url'])
        df_ratings = db_jy.get_factor('cbond_ratings_external', code_list=cbond_innercode_list,
                                      field='cr.MainCode')
        mappings = df_ratings[['crcode', 'crdesc']].drop_duplicates()
        # 同一日的多个评级取最低
        df_ratings.sort_values(by=['cbond_code', 'crdate', 'crcode'], inplace=True)
        df_ratings = df_ratings.drop_duplicates(subset=['crdate', 'cbond_innercode'], keep='first')
        # 处理成时间序列
        df_ratings_mat = df_ratings.pivot(index='crdate', columns='cbond_innercode', values='crdesc')
        df_ratings_mat = df_ratings_mat.reindex(
            index=pd.date_range(df_ratings['crdate'].min(), max(df_ratings['crdate'].max(),
                                                                self.basedatetime)))
        df_ratings_mat = df_ratings_mat.fillna(method='ffill')
        df_ratings_ts = df_ratings_mat.stack().to_frame('crdesc').reset_index()
        df_ratings_ts = pd.merge(df_ratings_ts, mappings, left_on=['crdesc'], right_on=['crdesc'], how='left')
        df_ratings_ts = df_ratings_ts.rename(columns={'level_0': 'd_date', 'crdesc': 'rating', 'crcode': 'rating_code'})

        # 昨日评级
        df_ratings_mat_last = df_ratings_mat.shift()
        df_ratings_ts_last = df_ratings_mat_last.stack().to_frame('crdesc').reset_index()
        df_ratings_ts_last = pd.merge(df_ratings_ts_last, mappings, left_on=['crdesc'],
                                      right_on=['crdesc'], how='left').rename(
            columns={'level_0': 'd_date', 'crdesc': 'rating_last', 'crcode': 'rating_code_last'})

        # 评级变动
        # df_ratings_ts = pd.merge(df_ratings_ts, df_ratings_ts_last, on=['d_date', 'cbond_innercode'])
        df_ratings_ts = df_ratings_ts.sort_values(by=['cbond_innercode', 'd_date'])
        df_ratings_ts['rating_chg'] = df_ratings_ts.groupby('cbond_innercode')['rating_code'].transform('diff')
        df_ratings_ts['if_rating_down'] = df_ratings_ts['rating_chg'] < 0
        df_ratings_ts = pd.merge(self.basic_info[['cbond_innercode', 'cbond_windcode', 'cbond_code']], df_ratings_ts,
                                 on='cbond_innercode', how='right')
        df_ratings_ts = df_ratings_ts[df_ratings_ts['d_date'] == self.basedatetime]
        # 合并上昨日评级
        df_ratings_ts = pd.merge(df_ratings_ts,
                                 df_ratings_ts_last[['d_date', 'cbond_innercode', 'rating_last', 'rating_code_last']],
                                 on=['d_date', 'cbond_innercode'], how='left')
        return df_ratings_ts

    def monitor_rating_chg_risk(self):
        """
        监控评级下调风险
        """
        logger.info("监控评级下调风险")
        cbond_ratings = self.get_ratings(self.basic_info['cbond_innercode'].to_list())
        rating_down_risk, rating_down_risk_detail = self._cal_risk_pct(
            cbond_ratings[['d_date', 'cbond_code', 'if_rating_down']],
            'if_rating_down')
        rating_down_risk['risk_type'] = '评级下调风险'
        rating_down_risk_detail['risk_type'] = '评级下调风险'
        self.cbond_ratings = self._merge_with_holdings(cbond_ratings.drop(columns=['cbond_windcode']))
        self.rating_down_risk = rating_down_risk
        return rating_down_risk, rating_down_risk_detail
    ...
    def CalculateAll(self):
        ...
        # 4. 评级下调风险
        rating_down_risk, rating_down_risk_detail = self.monitor_rating_chg_risk()
        risk_ratio_detail_all = risk_ratio_detail_all.append(rating_down_risk_detail.drop(columns=['if_rating_down']))
        # 1. 个券赎回风险
        ...
        redeem_risk_data = self._merge_with_holdings(self.redeem_risk.drop(
            columns=['cbond_innercode', 'cbond_name', 'cbond_market', 'a_cbond_market',
                     'cbond_windcode']))  # 转换价值、转股溢价率、是否满足赎回条件等
        ...
        redeem_risk_data.rename(columns={'if_call_by_amount': '是否满足余额不足赎回条件',
                                         ...
                                         'if_in_stopexe': '是否处于赎回保护期',
                                         'reach_con_call_days': '满足条件赎回天数',
                                         'call_reg_dt': '赎回登记日',
                                         'call_status': '赎回状态',
                                         'call_status_des': '赎回状态描述',
                                         'if_call_by_price_adj': '是否满足价格赎回条件',
                                         'if_call_by_price_adj_bef': '是否满足价格赎回条件(早期)',
                                         'if_redeem_impale_premium': '是否触发强赎条款刺破溢价风险',
                                         'if_impale_prem_early': '是否触发(早期)强赎条款刺破溢价风险',
        ...
        #                               x: f"过去{x['callconditionday']}个交易日，有{x['callreachday']}"
        #                                  f"个交易日的收盘价不低于当期转股价格的{int(x['calllevel'] * 100)}%",
        #                                                   axis=1)
        redeem_risk_data['赎回条件'] = redeem_risk_data.apply(lambda
                                                              x: f"{x['callreachday']}/{x['callconditionday']},{int(x['calllevel'] * 100)}%",
                                                          axis=1)

        redeem_risk_data['触发进度'] = redeem_risk_data.apply(lambda x: "-/-" if x[
                                     '赎回状态'] == 'trigger_call'
                                    else f"{int(x['满足条件赎回天数'])}/{int(x['callreachday'])}",
                               axis=1)
        redeem_risk_data['触发百分比'] = redeem_risk_data.apply(lambda x: 1 if x[
                                     '赎回状态'] == 'trigger_call' else min(1, x['满足条件赎回天数'] / x['callreachday']), axis=1)
        ...
        if hasattr(self, 'special_clause'):
            ...
            self.special_clause_all = self.special_clause_all.drop(
                columns=['f_assetratio', 'cbond_code', 'cbond_windcode'])
            pass
    ...
    def _get_rc_cb_repay(self):
        ...
        df_risk = df_risk.copy()
        df_risk[['d_date', 'maturity']] = df_risk[['d_date', 'maturity']].applymap(
            lambda x: date_utils.strftime(x, '%Y-%m-%d'))
        # 持仓占比转换为小数
    ...
    def _get_rc_cb_deriv_data(self):
        ...
        mapping = {'是否满足余额不足赎回条件': 'if_remainmont_insufficient',
                   ...
                   '是否处于赎回保护期': 'if_in_stopexe',
                   '是否满足价格赎回条件(早期)': 'if_call_by_price_adj_bef',
                   '赎回登记日': 'call_reg_dt',
                   '赎回状态': 'call_status',
                   '赎回状态描述': 'call_status_des',
                   '是否触发(早期)强赎条款刺破溢价风险': 'if_impale_prem_early',
                   '触发进度': 'trigger_process',
                   '触发百分比': 'trigger_process_pct'}
        ...
        cols = ['d_date', 'c_fundname', 'cbond_name', 'cbond_windcode', 'f_asset', 'f_assetratio',
                ...
                'if_premium_agg', 'redeem_con_desc', 'if_in_stopexe', 'if_call_by_price_adj_bef',
                'call_reg_dt', 'call_status', 'call_status_des', 'if_impale_prem_early', 'trigger_process',
                'trigger_process_pct', 'ann_date_notcall', 'con_satisf_enddt', 'next_recount_dt',
                'ann_date_special_note',
                'ann_date_call']
        ...
        df_risk[['if_remainmont_insufficient', 'if_in_redeem_period', 'if_reach_price_cond',
                 ...
                 'if_premium_agg', 'if_impale_prem_early', 'if_call_by_price_adj_bef', 'if_in_stopexe',
                 'trigger_process_pct']] = \
            df_risk[['if_remainmont_insufficient', 'if_in_redeem_period', 'if_reach_price_cond',
                     'if_redeem_impale_premium', 'if_premium_shattered',
                     'if_premium_agg', 'if_impale_prem_early', 'if_call_by_price_adj_bef', 'if_in_stopexe',
                     'trigger_process_pct']].astype(float)
        df_risk.columns = list(map(lambda x: x.upper(), df_risk.columns))
        # df_risk = df_risk.convert_dtypes(convert_integer=False).copy()
        return df_risk
    ...
    def _get_rc_cb_price_bias_data(self):
        """
        转债和正股偏离
        """
        bias = self.speculate_bias.copy()
        bias = self._merge_with_holdings(bias.drop(columns=['cbond_windcode']))
        bias[['d_date']] = bias[['d_date']].applymap(lambda x: date_utils.strftime(x, '%Y-%m-%d'))
        # 处理成落库表格式
        df_risk = bias.copy()
        df_risk[['if_trigger_2', 'if_trigger_5', 'if_speculate']] = \
            df_risk[['if_trigger_2', 'if_trigger_5', 'if_speculate']].astype(float)
        # 持仓占比转换为小数
        df_risk['f_assetratio'] = df_risk['f_assetratio'] / 100
        df_risk.columns = list(map(lambda x: x.upper(), df_risk.columns))
        return df_risk

    def _get_rc_cb_rating_data(self):
        """
        转债外部评级表
        """
        ratings = self.cbond_ratings.copy()

        ratings[['d_date']] = ratings[['d_date']].applymap(lambda x: date_utils.strftime(x, '%Y-%m-%d'))
        # 处理成落库表格式
        df_risk = ratings.copy()
        df_risk['if_rating_down'] = df_risk['if_rating_down'].astype('float')
        # 持仓占比转换为小数
        df_risk['f_assetratio'] = df_risk['f_assetratio'] / 100
        df_risk.columns = list(map(lambda x: x.upper(), df_risk.columns))
        return df_risk
    ...
    def insert_2_db(self):
        ...
        # 4. RC_CB_REPAY(CBond_专户兑付风险)
        RC_CB_REPAY = self._get_rc_cb_repay()
        # 5.RC_CB_RISK_RATIO(CBond_风险比例)， 只包含触警的
        RC_CB_RISK_RATIO = self._get_rc_cb_risk_ratio()
        # 6. RC_CB_RISK_RATIO_DETAIL(CBond_风险比例明细)， 只包含触警的
        RC_CB_RISK_RATIO_DETAIL = self._get_rc_cb_risk_ratio_detail()
        # 7. RC_CB_DERIV_DATA(CBond_中间数据_衍生指标,  (添加字段))
        RC_CB_DERIV_DATA = self._get_rc_cb_deriv_data()
        # 8.RC_CB_LIQDAYS_DATA(CBond_中间数据_变现天数)
        RC_CB_LIQDAYS_DATA = self._get_rc_cb_liqdays_data()
        # 9.RC_CB_PRICE_BIAS(CBond_中间数据_和正股价格偏离）202402新增表
        RC_CB_PRICE_BIAS = self._get_rc_cb_price_bias_data()
        # 10.RC_CB_RATINGS(CBond_中间数据, 外部评级）202402新增表
        RC_CB_RATINGS = self._get_rc_cb_rating_data()
        table_names = ['RC_ALERT_CB_PORT', 'RC_CB_SEC_REDEEM', 'RC_CB_CLAUSE', 'RC_CB_REPAY',
                       ...
                       'RC_CB_PRICE_BIAS', 'RC_CB_RATINGS']
        datas = [RC_ALERT_CB_PORT, RC_CB_SEC_REDEEM, RC_CB_CLAUSE, RC_CB_REPAY,
                 RC_CB_RISK_RATIO, RC_CB_RISK_RATIO_DETAIL, RC_CB_DERIV_DATA, RC_CB_LIQDAYS_DATA, RC_CB_PRICE_BIAS,
                 RC_CB_RATINGS]
    ...
    def saveAll(self, save_dir):
        ...
        res_special_clause_risk['持仓市值'] = res_special_clause_risk.apply(lambda x: f"{round(x['f_asset'] / 10000)}万",
                                                                        axis=1)
        ...
        ratings = self.cbond_ratings.copy()
        ratings['持仓占比'] = ratings['f_assetratio'] / 100
        ratings['持仓市值'] = ratings.apply(lambda x: f"{round(x['f_asset'] / 10000)}万", axis=1)

        # 投机炒作风险价格偏离表
        bias = self.speculate_bias.copy()
        bias = pd.merge(self.basic_info[['cbond_windcode', 'cbond_name']], bias, on='cbond_windcode', how='right')

        # 2025/2/12: 增加转债研究意见表
        research_sec_aler = self.monitor_research_alert_sec(self.basedate)
        ...
        bias.to_excel(writer, sheet_name='CBond_中间数据_投机炒作风险')
        ratings.to_excel(writer, sheet_name='CBond_中间数据_评级变动')
        research_sec_aler.to_excel(writer, sheet_name='CBond_转债研究风险提示', index=False)
        writer.close()
    ...
    @classmethod
    def monitor_research_alert_sec(cls, t):
        """
        监控转债研究风险券
        """
        UPPER_THRED = 0.5
        db_risk = DbFactor(config['data_base']['QUANT']['url'])
        sec_risk = db_risk.get_factor('cbond_research_conclusion_holds', code_list='all', field='a.c_date',
                                      t=f'\"{t}\"')
        sec_risk['upper_thred'] = np.nan
        sec_risk.loc[sec_risk['monitor_risk'] == '二级风险', 'upper_thred'] = UPPER_THRED

        sec_risk['risk_type'] = '未触警'
        cond_1 = ((sec_risk['monitor_risk'] == '二级风险') & (sec_risk['mv_nav_ratio'] >= sec_risk['upper_thred']))
        cond_2 = (sec_risk['monitor_risk'] == '三级风险(1)')
        cond_3 = (sec_risk['monitor_risk'] == '三级风险(2)')
        sec_risk.loc[cond_1, 'risk_type'] = '早期预警'
        sec_risk.loc[cond_2, 'risk_type'] = '重点预警'
        sec_risk.loc[cond_3, 'risk_type'] = '红灯预警'

        sec_risk['risk_info'] = np.nan
        sec_risk.loc[cond_1, 'risk_info'] = f'持仓个券触发固收研究部二类风险，且持仓占比超{UPPER_THRED}%'
        sec_risk.loc[cond_2, 'risk_info'] = f'持仓个券触发固收研究部三级风险(1)'
        sec_risk.loc[cond_3, 'risk_info'] = f'持仓个券触发固收研究部三级风险(2)'
        return sec_risk
